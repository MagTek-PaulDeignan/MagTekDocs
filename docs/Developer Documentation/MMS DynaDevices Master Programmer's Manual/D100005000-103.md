...
---
{% raw %}
<!-- ===== MagTek: tables + payload copy cards (drop once on the page) ===== -->
<style>
/* Shaded tables */
...
th,td{padding:.6rem .8rem;border-bottom:1px solid #eef2f7}
tbody tr:nth-child(odd){background:#fbfdff}

/* Copyable payload cards */
...
.mt-card .mt-pill{position:absolute;top:8px;left:8px;font:600 11px/1 system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#fff;background:#1565c0;padding:6px 10px;border-radius:999px}
.mt-card.response .mt-pill{background:#263238}
.mt-card .mt-copy{position:absolute;top:8px;right:8px;border:1px solid #2b2f36;background:#171b21;color:#cdd9e5;font-size:12px;border-radius:6px;padding:6px 10px;cursor:pointer}
.mt-card .mt-copy:hover{background:#1e242c}
.mt-card .mt-copy.copied{background:#1b6f3a;border-color:#1b6f3a;color:#fff}
.mt-card pre{margin:0;overflow:auto}
.mt-card code{display:block;color:#e6edf3;background:linear-gradient(180deg,rgba(255,255,255,.02),rgba(255,255,255,0));
  font:13px/1.55 ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace;
  padding:14px 16px;white-space:pre}
</style>

<script>
(function(){
  function extractLabel(classList){
    const cls = [...classList].join(' ');
    const m = /payload[-_\s]?(request|response)/i.exec(cls);
    return (m ? m[1] : 'request').toUpperCase();
  }
  function copyText(text, btn){
    ...
    function flash(b){ b.classList.add('copied'); b.textContent='Copied';
      setTimeout(()=>{ b.classList.remove('copied'); b.textContent='Copy'; },1200); }
  }
  function wrapBlock(codeEl, label){
    const pre = codeEl.closest('pre') || codeEl.parentElement;
    const outer = pre.parentElement; // Rouge wraps <div class="highlight"><pre><code>…</code></pre></div>
    const wrap = document.createElement('div');
    wrap.className = 'mt-card ' + (label==='RESPONSE' ? 'response' : 'request');
    const pill = document.createElement('span'); pill.className='mt-pill'; pill.textContent=label;
    const btn = document.createElement('button'); btn.className='mt-copy'; btn.type='button'; btn.textContent='Copy';
    btn.addEventListener('click', ()=>copyText(codeEl.textContent.trim(), btn));
    // Insert wrapper and move highlight block inside it
    outer.parentNode.insertBefore(wrap, outer);
    wrap.appendChild(pill); wrap.appendChild(btn); wrap.appendChild(outer);
  }

  document.addEventListener('DOMContentLoaded', function(){
    // Find both styles: class on wrapper div or on <code>
    const candidates = new Set([
      ...document.querySelectorAll('[class*="language-payload"]'),
      ...document.querySelectorAll('pre code[class*="language-payload"]')
    ]);
    candidates.forEach(function(el){
      const code = el.matches('code') ? el : el.querySelector('code');
      if(!code) return;
      const label = extractLabel(el.classList || code.classList);
      // Avoid double-wrapping
      if(code.closest('.mt-card')) return;
      wrapBlock(code, label);
    });
  });
})();
</script>
