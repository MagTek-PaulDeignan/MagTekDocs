...
nav_order: 
---
<div class="payload request mt-copyable">
  <span class="pill">REQUEST</span>
  <button type="button" class="copy-btn" data-target="req-msr">Copy</button>
  <pre><code id="req-msr">9F02060000000010009A032508219C01005F2A020840DF010101DF0202003C</code></pre>
</div>

<div class="payload response mt-copyable">
  <span class="pill">RESPONSE</span>
  <button type="button" class="copy-btn" data-target="resp-msr">Copy</button>
  <pre><code id="resp-msr">900057135413330089012345D25122011234567890F5A085413330089012345</code></pre>
</div>
...
.payload{
  ...
  background:#0f1216;  /* dark shading */
}
.payload code{
  display:block; color:#e6edf3; font-size:13px; line-height:1.55;
  font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono","Courier New", monospace;
}
.payload .pill{
  position:absolute; top:-10px; left:12px;
  background:#0a84ff; color:#fff; font-size:.7rem; font-weight:700;
  border-radius:999px; padding:.15rem .55rem;
}
.payload.response .pill{ background:#c8102e }  /* MagTek red for responses */
...
(function () {
  ...
  function getTarget(el) {
    ...
    var id = t.charAt(0) === '#' ? t.slice(1) : t;
    return document.getElementById(id);
  }

  function extractText(node) {
    return (node && node.textContent) ? node.textContent.replace(/\s+$/,'') : '';
  }

  function onClick(e) {
    var btn = e.target.closest('.copy-btn');
    if (!btn) return;
    var tgt = getTarget(btn);
    if (!tgt) return;
    var text = extractText(tgt).trim();
    doCopy(text, btn);
  }
  ...
  ready(function(){ document.addEventListener('click', onClick); });
})();
</script>
