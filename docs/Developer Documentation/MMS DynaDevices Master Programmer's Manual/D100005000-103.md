...
---
<style>
.mt-card{position:relative;margin:12px 0;padding:14px 16px;border:1px solid #2b2f36;border-radius:10px;background:#0f1216}
.mt-card pre{margin:0;overflow:auto}
.mt-card code{display:block;color:#e6edf3;font-size:13px;line-height:1.55;font-family:ui-monospace,Menlo,Consolas,monospace}
.mt-card .pill{position:absolute;top:-10px;left:12px;font-size:.72rem;font-weight:700;color:#fff;border-radius:999px;padding:.18rem .6rem}
.mt-card.request .pill{background:#0a84ff}
.mt-card.response .pill{background:#c8102e}
.mt-copy{position:absolute;top:8px;right:8px;border:1px solid #2b2f36;border-radius:6px;background:#171b21;color:#cdd9e5;font-size:12px;padding:6px 10px;cursor:pointer}
.mt-copy:hover{background:#1e242c}
.mt-copy.copied{background:#1b6f3a;color:#fff}
.mt-table table{width:100%;border-collapse:collapse;background:#0f1216;color:#e6edf3;border:1px solid #2b2f36;border-radius:10px;overflow:hidden}
.mt-table thead th{background:#11161c;color:#e6edf3;font-weight:600;border-bottom:1px solid #2b2f36;padding:.6rem .7rem}
.mt-table tbody td{padding:.55rem .7rem;border-bottom:1px solid #20242b}
.mt-table tbody tr:nth-child(odd){background:#11151a}
...
(function(){
  ...
  document.addEventListener('click',function(e){
    const btn=e.target.closest('.mt-copy');if(!btn)return;
    const id=btn.getAttribute('data-target');const node=document.getElementById(id);if(!node)return;
    copyText(node.innerText.trim(),btn);
  });
})();
</script>
<div class="mt-card request">
  <span class="pill">REQUEST</span>
  <button class="mt-copy" data-target="req1">Copy</button>
  <pre><code id="req1">9F02060000000010009A032508219C01005F2A020840DF010101DF0202003C</code></pre>
</div>

<div class="mt-card response">
  <span class="pill">RESPONSE</span>
  <button class="mt-copy" data-target="resp1">Copy</button>
  <pre><code id="resp1">900057135413330089012345D25122011234567890F5A085413330089012345</code></pre>
</div>
<div class="mt-table">
